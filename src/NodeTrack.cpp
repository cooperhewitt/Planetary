...
// TRACK INFORMATION
	...
	mPlanetTexIndex			= (int)( mNormPlayCount * ( G_NUM_PLANET_TYPES * G_NUM_PLANET_TYPE_OPTIONS - 1 ) );
...
void NodeTrack::drawClouds( const vector<gl::Texture> &clouds )
{
	if( mSphereScreenRadius > 2.0f && mDistFromCamZAxis < -0.005f ){
		if( mCamDistAlpha > 0.05f && mIsMostPlayed ){
			...
			// LIT CLOUDS
				gl::pushModelView();
				radius = mRadius * mDeathPer + 0.00005f;
				gl::scale( Vec3f( radius, radius, radius ) );
				glEnable( GL_RESCALE_NORMAL );
				gl::rotate( mMatrix );
				gl::rotate( Vec3f( 0.0f, 0.0f, mAxialTilt ) );
...
//			alpha = pinchAlphaPer;
		...
		Vec2f radius = Vec2f( mRadius * stretch, mRadius ) * 2.55f;
		tex.enableAndBind();
...
//		}
//		
		// TODO: MAKE THIS SLOWLY BLINK IF PAUSED?
		float alpha = 0.4f;
